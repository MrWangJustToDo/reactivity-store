...
const useCountState_v3 = createState(
  withActions(() => ({ count: 1 }), {
    generateActions: (state) => {
      return {
        add: (v) => {
...
const useCountState_v4 = createState(
  withNamespace(() => ({ count: 1 }), { namespace: "foo_2", reduxDevTool: true }),
  {
    withActions: (state) => {
      return {
        add: async (v) => {
        ...
        del: async () => state.count--,
      };
    },
    withNamespace: "count_4",
  }
);
