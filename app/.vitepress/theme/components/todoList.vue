...
const AllComplete = () => {
  ...
  return createElement(
    "span",
    {
      className: "allCheck" + (isAllComplete ? " checked" : ""),
    ...
    item.edit
      ...
      : createElement(
          ...
          createElement(
            ...
            createElement("input", { type: "checkbox", onClick: () => data.toggleTodo(item.id) })
          ),
          createElement(
            "p",
            { className: "content", onDoubleClick: () => data.enableEdit(item.id) },
            item.text,
            createElement("i", { className: "close", onClick: () => data.removeTodo(item.id) })
          )
        )
  );
};
...
const Message = () => {
  ...
  return createElement(
    "div",
    { className: "msg", ["data-msg"]: notDoneLength < 2 ? notDoneLength + " item" : notDoneLength + " items" },
    createElement(
      "span",
      { className: "toolBtns" },
    ...
    createElement(
      ...
      createElement(
        ...
        createElement(
          Fragment,
          null,
          currentShow.map((item) => createElement(Item, { key: item.id, item: item }))
        ),
